...
We leave the details of TensorFlow installation in a separate document:[TensorFlow Installation](./TENSORFLOW_INSTALL.md), please refer to it if needed.
...
After Installing Rosetta, we can test whether it works or not. We can do this by building a demo of the [millionaire problem](../example/millionaire/README.md). `Rosetta` uses a 3-party `MPC` model that requires the deployment of three computing nodes, which can be deployed on real multiple machines or be simulated in s single machine with multi-processes.

> `Rosetta` is based on TensorFlow. Currently, it only supports non-distributed graph execution.

### Example

Here we use the famous [demo of millionaire's problem](../example/millionaire/README.md) as our example.
